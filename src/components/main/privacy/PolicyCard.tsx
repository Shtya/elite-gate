...
export default function PolicyCard({ title, items }: PolicyCardProps) {
  ...
  return (
    <section aria-labelledby={sectionId} className='  group col-span-12 md:col-span-6 xl:col-span-4 rounded-2xl border border-neutral-200 bg-white p-5 lg:p-7 transition-all hover:-translate-y-0.5 hover:shadow-xl focus-within:shadow-xl'>
      {/* Accent */}
      <span className='block h-1 w-16 rounded-full bg-gradient-to-l from-primary to-tertiary opacity-80 mb-4' />

      <h3 id={sectionId} className='text-2xl font-bold text-neutral-900 mb-3'>
        {title}
      </h3>

      <ul id={listId} role='list' className='flex flex-col gap-3 leading-7 text-neutral-700'>
        {items.map((item, idx) => (
